...
// 런타임 역할 선택 및 student -> CU 라우팅에 공통으로 사용하는 CU 노드 ID
#define L3_CU_ID                         0
...
#define L3_RSSI_THRESHOLD               -50  // 출석 인정 RSSI 임계값 (dBm)

// 학생별 RSSI를 이 횟수만큼 모은 뒤 평균으로 출석 여부를 판단
#define L3_RSSI_SAMPLE_COUNT             3

// [출석 창 유지 시간]
// CU가 'start'를 입력하면 이 시간(초) 동안 출석을 받고, 이후 자동으로 닫힘
// 실습: 10으로 낮추면 10초 만에 출석 창이 닫혀서 빠른 테스트 가능
#define L3_ATTEND_WINDOW_SEC            60  // 출석 창 유지 시간 (초, 기본 10분)

// [사전 경고 타이머]
// 출석 마감 이 시간(초) 전에 아직 미출석인 학생에게 경고 메시지 브로드캐스트
// 반드시 L3_ATTEND_WINDOW_SEC보다 작아야 함
#define L3_PRE_DEADLINE_ALERT_SEC       30  // 마감 전 경고 시점 (초, 기본 5분 전)
...
#define L3_LEAVE_GRACE_SEC              30   // 이탈 유예 시간 (초, 기본 20초)
